...
<div class="card">
    <div class="card-body">
        <form id="documentForm" method="POST" action="{{ route('documents.update', $document) }}">
            @csrf
            @method('PUT')
            ...
            <div class="row g-3">
                ...
                <div class="col-12">
                    ...
                    <input type="text" class="form-control" value="{{ $document->reference_no }}" disabled>
                ...
                <div class="col-12">
                    ...
                    <select class="form-select @error('template_id') is-invalid @enderror" 
                        ...
                        @foreach($templates as $template)
                            <option value="{{ $template->id }}" {{ old('template_id', $document->template_id) == $template->id ? 'selected' : '' }}>
                                {{ $template->name }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div class="col-md-6">
                    ...
                    <input type="text" class="form-control @error('recipient_name') is-invalid @enderror" 
                           id="recipient_name" name="recipient_name" value="{{ old('recipient_name', $document->recipient_name) }}" required>
                ...
                <div class="col-md-6">
                    ...
                    <input type="text" class="form-control @error('subject') is-invalid @enderror" 
                           id="subject" name="subject" value="{{ old('subject', $document->subject) }}" required>
                    @error('subject')
                        <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
                </div>
                ...
                <div class="col-12">
                    ...
                    <textarea class="form-control @error('recipient_address') is-invalid @enderror" 
                              id="recipient_address" name="recipient_address" rows="3" required>{{ old('recipient_address', $document->recipient_address) }}</textarea>
                ...
                <div class="col-12">
                    ...
                    <textarea class="form-control @error('body_html') is-invalid @enderror" 
                              id="body_html" name="body_html" rows="15" required>{{ old('body_html', $document->body_html) }}</textarea>
                    @error('body_html')
                        <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
                </div>
    ...
    document.getElementById('documentForm').addEventListener('submit', function(e) {
        ...
        .then(data => {
            if (data.success) {
                ...
                submitBtn.innerHTML = '<i class="bi bi-check-circle me-1"></i> Update Document';
            }
        })
        .catch(error => {
